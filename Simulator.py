...
class Simulator:
    ...
    def simulate_case(self, test_idx, full_model=False, BldPitch1_model=None, computed_derivatives=False, is_discrete=False):
        print(f'\nSimulating {self.key} for case {test_idx}')

        # full_lib.library_ensemble = False

        x0_train = self.data_processor.x_train[self.key][test_idx][0, :]
        x0_test = self.data_processor.x_test[self.key][test_idx][0, :]

        if self.key == 'P_motor' and full_model:
            # if the bladepitch derivatives were computed from a BldPitch1 model simulation via the central difference method, we need to truncate values
            if computed_derivatives:

                u_train_dash = np.array(self.data_processor.u_train[self.key][test_idx][1:])
                u_test_dash = np.array(self.data_processor.u_test[self.key][test_idx][:-1])

                for drvt in ['', '_dot', '_ddot']:
                    u_train_dash[:, self.data_processor.ctrl_inpt_cols['P_motor'].index(f'BldPitch1{drvt}')] = \
                        BldPitch1_model[test_idx][f'BldPitch1{drvt}'][:len(self.data_processor.t_train) - 1]

                for drvt in ['', '_dot', '_ddot']:
                    u_test_dash[:, self.data_processor.ctrl_inpt_cols['P_motor'].index(f'BldPitch1{drvt}')] = \
                        BldPitch1_model[test_idx][f'BldPitch1{drvt}'][len(self.data_processor.t_train) - 1:]

                x0_train = self.data_processor.x_train[self.key][test_idx][1, :]
                t_train = self.data_processor.t_train[1:]
                t_test = self.data_processor.t_test[:-1]
                t_full = np.concatenate([t_train, t_test])

            else:
                x0_train = self.data_processor.x_train[self.key][test_idx][0, :]

                u_train_dash = self.data_processor.u_train[self.key][test_idx]
                u_test_dash = self.data_processor.u_test[self.key][test_idx]

                for drvt in ['', '_dot', '_ddot']:
                    u_train_dash[:, self.data_processor.ctrl_inpt_cols['P_motor'].index(f'BldPitch1{drvt}')] = \
                        BldPitch1_model[test_idx][f'BldPitch1{drvt}'][:len(self.data_processor.t_train)]

                for drvt in ['', '_dot', '_ddot']:
                    u_test_dash[:, self.data_processor.ctrl_inpt_cols['P_motor'].index(f'BldPitch1{drvt}')] = \
                        BldPitch1_model[test_idx][f'BldPitch1{drvt}'][len(self.data_processor.t_train):]

            # self.models[self.key].simulate(x0_train, len(t_train) - 2, u=u_train_dash)
            # x_sim_test = self.models[self.key].simulate(x0_test, len(t_test) - 2, u=u_test_dash)
        ...
        # x_sim_train2 = self.models[self.key].simulate(x0_train, t_train, u=u_train_dash)
        # x_sim_test2 = self.models[self.key].simulate(x0_test, len(t_test), u=u_test_dash)

        if not is_discrete:

            u_fun_train = interp1d(
                self.data_processor.t_train, u_train_dash, axis=0, kind="cubic", fill_value="extrapolate"
            )
            u_fun_test = interp1d(
                self.data_processor.t_test, u_test_dash, axis=0, kind="cubic", fill_value="extrapolate"
            )
            t_train = self.data_processor.t_train[:-1]
            t_test = self.data_processor.t_test[:-1]
            t_full = np.concatenate([t_train, t_test])
            ...
            def rhs_train(t, x):
                # return self.models[self.key].predict(x[np.newaxis, :], u_fun(t))[0]
                # uk = u_train_dash[k - 1, :]
                ...
                xk = x[np.newaxis, :]
                x_shape = np.shape(xk)
                uk = u_fun_train(t).reshape(1, -1)
                X = np.concatenate((xk, uk), axis=1)  # .reshape(x_shape)
                Xt = X
                for _, name, transform in self.models[self.key].model._iter(with_final=False):
                    Xt = transform.transform(Xt)

                return (safe_sparse_dot(Xt, self.models[self.key].model.steps[-1][1].optimizer.coef_.T, dense_output=True)
                        + self.models[self.key].model.steps[-1][1].optimizer.intercept_).reshape(x_shape)[0]
            ...
            x_sim_train = (
                (solve_ivp(rhs_train, (self.data_processor.t_train[0], self.data_processor.t_train[-1]), x0_train,
                           t_eval=self.data_processor.t_train)).y
            ).T
            # x_sim_train = np.hstack([x_sim_train, self.models[self.key].predict(x_sim_train, u_train_dash[:-1, :])])
            x_sim_train = np.hstack([x_sim_train,
                                     np.vstack([rhs_train(t0, x0) for x0, t0 in zip(x_sim_train, self.data_processor.t_train)])
                                     ])

            x_sim_test = (
                (solve_ivp(rhs_test, (self.data_processor.t_test[0], self.data_processor.t_test[-1]), x0_test, t_eval=self.data_processor.t_test)).y
            ).T
            # x_sim_test = np.hstack([x_sim_test, self.models[self.key].predict(x_sim_test, u_test_dash[:-1, :])])
            x_sim_test = np.hstack([x_sim_test,
                                    np.vstack([rhs_test(t0, x0) for x0, t0 in zip(x_sim_test, self.data_processor.t_test)])
                                    ])

        else:

            x_sim_train = np.zeros((len(self.data_processor.t_train), len(x0_train)))
            x_sim_train[0, :] = x0_train

            for k in range(1, len(self.data_processor.t_train)):
                xk = x_sim_train[k - 1, :]
                uk = u_train_dash[k - 1, :]
                ...
                x_sim_train[k, :] = safe_sparse_dot(Xt,
                                                    self.models[self.key].model.steps[-1][1].optimizer.coef_.T, dense_output=True) \
                                    + self.models[self.key].model.steps[-1][1].optimizer.intercept_

            x_sim_test = np.zeros((len(self.data_processor.t_test), len(x0_test)))
            x_sim_test[0, :] = x0_test

            for k in range(1, len(self.data_processor.t_test)):
                xk = x_sim_test[k - 1, :]
                uk = u_test_dash[k - 1, :]
                Xt = np.concatenate((xk, uk))[np.newaxis, :]
                for _, name, transform in self.models[self.key].model._iter(with_final=False):
        ...
        for case_idx in range(self.data_processor.n_datasets):
            ...
            if case_idx in test_indices:
                ...
                if self.data_processor.model_per_wind_speed:
                    key = f'{self.key}_{self.data_processor.data_all["vmean"][case_idx]}'
                else:
                    key = self.key

                # BldPitch1_sim[case_idx] = \
                yield self.simulate_case(case_idx, **kwargs)
            else:
                yield None

    def save_simulations(self, results_dir, sims_list):
        with open(os.path.join(results_dir, f'{self.key}_sim.txt'), 'wb') as fh:
            pickle.dump(sims_list, fh)

    def load_simulations(self, results_dir):

        with open(os.path.join(results_dir, f'{self.key}_sim.txt'), 'rb') as fh:
